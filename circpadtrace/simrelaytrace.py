...
def estimate_latency_in_us(infname):
    ...
    and middle relay. We use this together with sampling uniformly
    [-LATENCY_NOISE_MS/2,LATENCY_NOISE_MS/2] as noise on each estimate.
    '''
    ...
    def sample():
        noise = np.random.uniform(-1*LATENCY_NOISE_MS/2, LATENCY_NOISE_MS/2)
        s = int(latency + noise)
        return s
